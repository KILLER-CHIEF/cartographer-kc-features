...
{
	pLog.WriteLog("Spanwing Object(Object_Datum Index : %X)", datumindex);
...
unsigned __int16 H2class::SpawnObjAtCamera(int datumindex)
{
	...
	nObject[9] = (*(float*)(((char*)game.GetBase()) + 0x4A850C) + 2.8f);//z
...
int __cdecl Game:: call_object_placement_data_new(void* s_object_placement_data, int object_definition_index, int object_owner, int unk)
{
	...
	void*__cdecl Game::call_CreateCharacter(int CharDatumIndex, signed __int16 a2, void* a3)
	{
		pLog.WriteLog("CreateCharacter(%X,%X,%X)", CharDatumIndex, a2, a3);
